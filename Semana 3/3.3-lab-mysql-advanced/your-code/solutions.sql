...
select au_id as "AUTHOR ID", au_lname as "LAST NAME", au_fname as "FIRST NAME", round(sum(advance *(rt/100) + ROYALTIES),2) as PROFITS from (
	select title_id, au_id, au_lname, au_fname, advance, rt ,sum(ROYALTIES) as ROYALTIES from (
		select t.title_id, t.price, t.advance, t.royalty, s.qty, a.au_id, au_lname, au_fname, ta.royaltyper as rt, (t.price * s.qty * t.royalty * ta.royaltyper / 10000) as ROYALTIES
		from titles t
		inner join sales s on s.title_id = t.title_id
		inner join titleauthor ta on ta.title_id = s.title_id
		inner join authors a on a.au_id = ta.au_id
...
select tmp2.au_id as "AUTHOR ID", a.au_lname as "LAST NAME", a.au_fname as "FIRST NAME", round(sum(t.advance *(rt/100) + ROYALTIES),2) as PROFITS 
from tmp2
